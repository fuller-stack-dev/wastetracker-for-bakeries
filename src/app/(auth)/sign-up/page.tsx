...
export default function SignUpPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (hasClerk) {
      setError("Clerk sign-up will be integrated when Clerk keys are configured.");
      setLoading(false);
      return;
    }
  ...
  async function handleGitHubSignUp() {
    if (hasClerk) {
      setError("GitHub sign-up will be available when Clerk is configured.");
      return;
    }
    router.push("/dashboard");
  }
  ...
  return (
    <div className="w-full max-w-md mx-auto p-6">
      ...
      <p className="text-center text-sm text-[var(--muted-foreground)] mt-6">
        ...
        <Link href="/sign-in" className="text-[var(--color-sienna)] font-medium hover:underline">
          Sign in
        </Link>
      </p>
    </div>
